...
## [1.34.0] - 2026-09-25
- Inventar: Ein Artikel kann jetzt mehreren Gruppen gehören (bisher genau einer Gruppe oder keiner). So nutzen z. B. Hausmeister und Toilettenbetreuung dasselbe Inventar: beide sehen dieselben Artikel und denselben Bestand, Buchungen der einen sind für die andere sofort sichtbar, und bei niedrigem Bestand werden beide Gruppen benachrichtigt (sobald mindestens eine gerade im Dienst ist, dann aber alle). Im Artikel-Formular (Verwaltung und Inventar-Seite) gibt es dafür statt der Einzelauswahl jetzt Gruppen-Chips zum Mehrfach-Anklicken; keine Gruppe bedeutet wie bisher „für alle sichtbar“.
  - Neue Sammelzuordnung unter Verwaltung → Inventar („Inventar gemeinsam nutzen“, nur Admin): allen Artikeln einer Gruppe eine weitere Gruppe hinzufügen, ohne jeden Artikel einzeln zu bearbeiten – nur Hinzufügen, nie Entfernen.
  - Anzeige: Karten zeigen alle Gruppen als Badges; für Admins/Schichtleiter stehen gemeinsame Artikel in einem gemeinsamen Abschnitt („Hausmeister · Toilettenbetreuung“) statt doppelt. „Sichtbarkeit anpassen“ blendet einen gemeinsamen Artikel nur aus, wenn alle seine Gruppen ausgeblendet sind.
  - Bestehende Zuordnungen werden beim Start automatisch übernommen. Struktur-Export/-Import kennt die Gruppen-Liste (ältere Export-Dateien mit einer Gruppe werden weiter gelesen). Die alte Spalte `inventory_items.group_id` wird als Spiegel der ersten Gruppe weitergeschrieben, damit ein Zurückrollen auf eine ältere Version nicht alle Zuordnungen verliert.

## [1.33.0] - 2026-09-24
- Feedback: Der Admin bekommt jetzt mit, wenn jemand über den schwebenden Feedback-Button einen Bug oder Wunsch meldet, ohne täglich in die Feedback-Liste schauen zu müssen. Bisher löste das Absenden gar keine Benachrichtigung aus. Unter Verwaltung → Benachrichtigungen → „Feedback-Benachrichtigung“ lässt sich ein Kanal (Gotify, ntfy oder Signal) wählen, an den bei jedem neuen Feedback eine Nachricht mit Titel, Absender, Priorität und Beschreibung geht (bei Priorität „Hoch“ mit erhöhter Priorität; Screenshots stehen weiter nur in der Verwaltung). Dafür genügt ein eigener Kanal ohne Gruppen-Zuordnung. „Testnachricht senden“ prüft Token und Basis-URL. Ohne gewählten Kanal ändert sich nichts; wird der gewählte Kanal gelöscht, wird die Auswahl automatisch geleert. Intern: der Versand je Kanal ist jetzt eine eigene Funktion (`send_to_channel`), die auch die Gruppen-Benachrichtigungen nutzen (Gotify kennt dabei nun auch erhöhte Prioritäten; bisher war sie fest 5). Neue Spalte `app_settings.feedback_channel_id` wird beim Start angelegt.

## [1.32.0] - 2026-09-24
- Meldungen: Kommentarzwang beim Erledigen. Rückmeldung aus dem Betrieb: Meldungen wurden ohne jede Angabe auf „Erledigt“ gestellt. Wer eine Meldung jetzt auf „Erledigt“ setzt, bekommt einen Dialog „Was wurde gemacht?“ und muss eine Kurzbeschreibung eingeben (mindestens 5 Zeichen, optional mit Foto, z. B. als Nachweis). Erst damit wird der Status gespeichert; „Abbrechen“ setzt das Auswahlfeld auf den alten Status zurück. Die Beschreibung erscheint als Kommentar im Thread, im Verlauf und direkt auf der erledigten Karte („Erledigt: …“), und die Push-Benachrichtigung „Erledigt“ enthält sie ebenfalls. Gilt für alle Nutzer inklusive Admins und wird zusätzlich serverseitig durchgesetzt (nicht nur im Dialog). „In Bearbeitung“ und „Wieder öffnen“ brauchen weiterhin keine Angabe; beim Wiederöffnen verschwindet die Erledigt-Notiz von der Karte (der Kommentar bleibt als Historie). Bereits erledigte Meldungen bleiben unverändert. Neue Spalte `reports.resolved_note` wird beim Start automatisch angelegt.
...
## [1.28.0] - 2026-09-20
- Sicherungen enthalten jetzt auch die Bilder. Bisher steckte nur die SQLite-Datenbank in den Sicherungen; die hochgeladenen Bilder (Meldungs-, Inventar-, Aufbau-, Feedback-Fotos, Profilbilder unter `uploads/`) fehlten in jeder Sicherung – nach einem Verlust des Datenverzeichnisses, einem Server-Umzug oder einer Wiederherstellung aus der Nextcloud wären alle Bilder weg gewesen (die Einträge hätten kaputte Bilder gezeigt).
  - Manueller Download: standardmäßig ein ZIP mit Datenbank und allen Bildern; „Nur Datenbank (.db)“ bleibt als eigener Link verfügbar. Der Audit-Log-Eintrag heißt jetzt korrekt „Datenbank + Bilder“ bzw. „nur .db“.
  - Bild-Archiv: einmal täglich (erster Sicherungslauf des Tages, nur wenn Bilder vorhanden sind) entsteht neben den DB-Sicherungen ein `bilder-JJJJMMTT-HHMMSS.zip` in `backups/`, mit derselben Aufbewahrung. Anzeige und Download unter Verwaltung → System („Bild-Archive“), dazu die Anzahl/Größe der aktuellen Bilder.
  - Nextcloud: Bilder werden bei jedem Sicherungslauf einzeln in den Unterordner `uploads/<Bereich>/` des Zielordners hochgeladen. Der Remote-Ordner wird je Bereich einmal aufgelistet, Dateien mit gleichem Namen und gleicher Größe werden übersprungen – übertragen wird nur Neues. Ein großer Rückstand verteilt sich mit einem Zeitbudget von ca. 4 Minuten je Lauf auf mehrere Läufe, damit der Scheduler nicht blockiert. Es wird nie etwas in der Nextcloud gelöscht (auch nicht bei in ClubHUB gelöschten Bildern); die Aufbewahrung gilt weiter nur für die `auto-*.db`. Eigener Status („Bilder“) und eigene Fehlerwarnung auf der Nextcloud-Karte; ein Fehler dort beeinträchtigt weder den DB-Upload noch die lokalen Sicherungen. Neue Spalten in `app_settings` werden beim Start automatisch angelegt.
  - Einspielen: „Manuelles Backup einspielen“ nimmt jetzt `.zip` und `.db`. ZIP mit Datenbank: Datenbank wird (wie bisher nach Sicherheitskopie und mit Neustart) ersetzt, Bilder werden ergänzt. ZIP nur mit Bildern (Bild-Archiv oder ein in der Nextcloud als ZIP heruntergeladener Ordner): Datenbank bleibt unverändert, kein Neustart. Bilder werden nur ergänzt, nie gelöscht; gleich große vorhandene Dateien bleiben unberührt. Abgesichert gegen Pfad-Traversal (nur `…/uploads/<Bereich>/<Datei>` mit einfachen Namen wird entpackt), eine ungültige Datenbank im ZIP bricht ab, bevor irgendetwas entpackt wird.
  - Die automatischen `auto-*.db`-Sicherungen und die Wiederherstellung einzelner Sicherungen ändern sich nicht (die Bilder liegen dort im selben Datenverzeichnis und bleiben erhalten).
- Entwicklung: `deploy-nas.sh` heißt jetzt `deploy-staging.sh` und deployt auf den neuen Docker-Host (Proxmox-VM) statt aufs NAS: das Image wird lokal gebaut, per `docker save | load` übertragen und der bestehende Stack neu gestartet (README aktualisiert).
...
## [1.23.0] - 2026-09-16
- Historie: Admins können jetzt neben "Erledigt von" auch Datum und Uhrzeit einer Erledigung nachträglich korrigieren (Stift-Symbol neben dem Datum). Wie beim Umtragen ausschließlich für Admins und im Audit-Log dokumentiert (alter und neuer Zeitpunkt).
...
- Bereichs-Übersicht (Dashboard/„Bereiche“, Desktop & Mobil): Bereiche, für die man selbst zuständig ist, werden jetzt farblich mit der eigenen Gruppenfarbe hervorgehoben (dieselben Farben, die auch bei den Gruppen-Badges verwendet werden) – auf einen Blick erkennbar, ohne jede Karte einzeln lesen zu müssen. Der bestehende grün/gelb/rot-Status je Bereich bleibt davon unabhängig und weiterhin sichtbar.
- Historie: Admins können jetzt direkt in der Tabelle per Dropdown ändern, wer eine Aufgabe tatsächlich erledigt hat – z.B. wenn versehentlich mehrere Erledigungen unter dem eigenen Admin-Konto statt dem des tatsächlich putzenden Mitarbeiters gespeichert wurden. Bisher blieb dafür nur Löschen und neu Anlegen. Die Änderung greift sofort ohne Neuladen und wird im Audit-Log protokolliert.
