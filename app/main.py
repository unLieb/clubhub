...
def _admin_system_context(
    ...
    settings = get_app_settings(db)
    # Kein eigenes Verlaufs-Widget auf dieser Karte mehr (siehe Git-Historie):
    # das allgemeine Audit-Log (Verwaltung -> Audit-Log, gespeist aus
    # denselben log_audit()-Aufrufen in admin_export_data/admin_import_data)
    # deckt Struktur-Export/-Import bereits vollstaendig ab, eine gefilterte
    # Vorschau direkt auf dieser Karte war schlicht redundant.
    return {
        "request": request,
        "user": admin,
